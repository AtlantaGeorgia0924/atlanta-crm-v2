...
BEGIN
    ...
    IF to_regclass('public.crm_audit_log') IS NOT NULL THEN
        INSERT INTO crm_audit_log (
            ...
            NULLIF(BTRIM(COALESCE(p_applied_by, '')), ''),
            jsonb_build_object(
                'paid_amount', v_prev_paid,
                'balance', v_prev_balance,
    ...
    IF to_regclass('public.cashflow_audit_log') IS NOT NULL THEN
        INSERT INTO cashflow_audit_log (
            action,
            amount,
            performed_by,
            related_record_id,
...
VALUES (
    '028_repair_payment_transaction_rpcs',
    'Repairs missing payment transaction RPCs and idempotency index',
    'completed',
    NOW(),
    jsonb_build_object(
        'functions', jsonb_build_array('apply_service_payment_tx', 'reverse_service_payment_tx'),
