...
CREATE OR REPLACE FUNCTION generate_unique_payment_reference(p_prefix TEXT)
RETURNS TEXT
LANGUAGE plpgsql
AS $$
BEGIN
    RETURN CONCAT(
        COALESCE(NULLIF(BTRIM(p_prefix), ''), 'ATL-PAY'),
        '-',
...
BEGIN
    ...
    IF v_reference IS NULL THEN
        v_reference := generate_unique_payment_reference('ATL-PAY');
    END IF;
    ...
    LOOP
        ...
        EXCEPTION WHEN unique_violation THEN
            ...
            IF COALESCE(v_constraint_name, '') <> 'payments_reference_no_key' THEN
                RAISE;
            END IF;

            v_reference_attempts := v_reference_attempts + 1;
            IF v_reference_attempts >= 5 THEN
                RAISE EXCEPTION 'Could not allocate a unique payment reference after % attempts', v_reference_attempts;
            END IF;
...
BEGIN
    ...
    v_reference := generate_unique_payment_reference('ATL-REV');
